...
@interface FastttCamera () <FastttFocusDelegate, FCFacialDetectorDelegate>
...
// Face detection
@property (nonatomic, strong) FCFacialGesturesDetector *facialGesturesDetector;
...
@property (nonatomic, readwrite) BOOL isImageSaved;
...
- (instancetype)init
{
    if ((self = [super init])) {
        ...
        // Face Detection
        _facialGesturesDetector = [FCFacialGesturesDetector new];
        _facialGesturesDetector.delegate = self;
//        _sampleRate = 0.5;
//        _isImageSaved = false;
    ...
    [self _checkDeviceAuthorizationWithCompletion:^(BOOL isAuthorized) {
        ...
        if (_deviceAuthorized) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                //----- Video Output used for Facial Gestures Detection ------
                // TODO
                // Check for error
                NSError* error = nil;
                [self.facialGesturesDetector startDetectionWithAVCaptureSession:_session atSampleRate:0.5 withError:&error];
                if (error != nil) {
                    NSLog(@"Failed to start facialGesturesDetector %@", [error description]);
                }
            });
        }
#if !TARGET_IPHONE_SIMULATOR
...
- (void)_teardownCaptureSession
{
    ...
    // Stop Facial Gestures Detectioin
    if (_facialGesturesDetector != nil) {
        [_facialGesturesDetector stopDetection];
    }
...
#pragma mark - FCFacialDetectorDelegate

-(void)didRegisterFacialGestureOfType:(FCGestureType)facialGestureType
{
    if ([self.delegate respondsToSelector:@selector(cameraController:didRegisterFacialGestureOfType:)]) {
        [self.delegate cameraController:self didRegisterFacialGestureOfType:facialGestureType];
    }
...
{
//    NSLog(@"Should update progress %.1f", progress);
    if ([self.delegate respondsToSelector:@selector(cameraController:didUpdateFacialGestureProgress:forType:)]) {
        [self.delegate cameraController:self didUpdateFacialGestureProgress:progress forType:facialGestureType];
    }
}
